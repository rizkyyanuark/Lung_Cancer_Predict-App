...
class App():
    def __init__(self, master):
        ...
        if self.file.exists():
            ...
            headers = ['Hari', 'Waktu', 'Nama Pasien', 'Umur', 'Jenis Kelamin',
                       ...
                       'Frequent Cold', 'Dry Cough', 'Snoring', 'Level', 'Penanganan']
        ...
        # Get the absolute path to the theme files
        forest_dark_path = os.path.abspath("src/forest-dark.tcl")
        forest_light_path = os.path.abspath("src/forest-light.tcl")
        ...
        try:
            ...
            raise RuntimeError(f"Failed to apply theme: {e}")
        ...
        self.notebook.add(self.tab1, text='FORM')
        self.notebook.add(self.tab2, text='GRAFIK')
        ...
        self.biodata_row = ttk.LabelFrame(
            self.frame, text="Personal Information")
        self.biodata_row.grid(row=0, column=0, padx=20, pady=10)

        self.bpjs_row = ttk.Labelframe(self.biodata_row, text='Additional')
        ...
        self.biodata_row1 = ttk.LabelFrame(
            self.frame, text="Complaint Details")
        ...
        complaint_row.add(test1, text='row 1')
        complaint_row.add(test2, text='row 2')
        complaint_row.add(test3, text='row 3')
        ...
        self.save_button = ttk.Button(test3, text="Predict",
        ...
        button = ttk.Button(self.biodata_row, text="Submit Form",
        ...
        self.biodata_row3 = ttk.LabelFrame(
            self.frame, text="Patients Waiting List")
        ...
        self.biodata_row4 = ttk.LabelFrame(
            self.frame, text="Patients Attended")
        ...
        # Define the columns
        cols = ("Level", 'Hari', "Waktu",
                "Nama Pasien", "Umur", "Jenis Kelamin")
        cols1 = ("Level", 'Day', "Time", "Name", "Age", "Gender")
        ...
        def calculate_max_width(treeview, df, cols):
            for col in cols:
                max_width = max(df[col].astype(str).map(len).max(), len(
                    col)) * 7  # Multiply by 7 for padding
                treeview.column(col, width=max_width)

        # Load the Excel file to calculate column widths
        df = pd.read_excel("data/data.xlsx")

        # Calculate and set the column widths for treeview
        calculate_max_width(self.treeview, df, cols)
        calculate_max_width(self.treeview2, df, cols)

        # Set the headings for treeview using cols1
        for col, col1 in zip(cols, cols1):
            self.treeview.heading(col, text=col1)
            self.treeview2.heading(col, text=col1)
    ...
    def update_chart(self):
        ...
        def create_new_charts():
            try:
                ...
                if hasattr(self, 'ax2'):
                    self.ax2.pie(sizes, labels=labels,
                                 autopct='%1.1f%%', startangle=90)
                    self.ax2.set_title("Berdasarkan \nJenis Kelamin")

                if hasattr(self, 'ax1'):
                    self.ax1.bar(self.total_categori.keys(),
                                 self.total_categori)
    ...
    # Insert row into Excel sheet
        ...
        row_values = [self.tanggal, self.time_string, self.name,
                      ...
                      self.chestpain_combobox.get(), self.coughingofblood_combobox.get(), self.fatigue_combobox.get(), self.weightloss_combobox.get(), self.shortnessofbreath_combobox.get(), self.wheezing_combobox.get(), self.swallowingdiffi_combobox.get(), self.clubbing_combobox.get(), self.frequentcold_combobox.get(), self.drycough_combobox.get(), self.snoring_combobox.get(), train, self.penanganan]
    ...
    def grafik(self):
        dfgraf = pd.read_excel('data/data.xlsx')
    ...
    def knn(self):
        ...
        # Load the model
        loaded_model = joblib.load('model/best_knn_model.pkl')
        ...
        self.hasilpredict = predict[0]
        messagebox.showinfo("Prediksi Level cancer", self.hasilpredict)
    ...
    def on_click(self, event):
        ...
        if answer == 'yes':
            ...
            values = list(self.treeview.item(selected_item)['values'])
            ...
            values[4] = int(values[4])  # Convert 'Umur' to integer
            # Find the row that matches the selected item and has 'Penanganan' as 'belum'
            mask = (df['Hari'] == values[1]) & (df['Waktu'] == values[2]) & (df['Nama Pasien'] == values[3]) & \
                (df['Umur'] == values[4]) & (df['Jenis Kelamin']
                                             == values[5]) & (df['Level'] == values[0])
            # Update the 'Penanganan' column for the matching row
            df.loc[mask, 'Penanganan'] = 'sudah'
    ...
    def priority_queue(self):
        ...
        for index, row in df.iterrows():
            q.put((self.order[row['Level']], row['Hari'], row['Waktu'],
                   row['Nama Pasien'], row['Umur'], row['Jenis Kelamin'], row['Penanganan']))
        ...
        for item in sorted(q.queue):
            ...
            if item[6] == 'belum':
                self.treeview.insert("", "end", values=tuple(item[:6]))
            else:
                self.treeview2.insert("", "end", values=tuple(item[:6]))
